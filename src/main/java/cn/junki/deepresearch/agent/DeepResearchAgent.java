...
public class DeepResearchAgent {
    ...
    public DeepResearchAgent(ChatModel chatModel) {
        ...
        this.largeResultEvictionInterceptor = LargeResultEvictionInterceptor.builder()
                .excludeFilesystemTools().toolTokenLimitBeforeEvict(5000).build();

        // 文件系统拦截器：允许读写操作（非只读模式）
        this.filesystemInterceptor = FilesystemInterceptor.builder().readOnly(false).build();
        ...
        this.toolRetryInterceptor = ToolRetryInterceptor.builder().maxRetries(1)
        ...
        this.summarizationHook = SummarizationHook.builder().model(chatModel)
                .maxTokensBeforeSummary(120000).messagesToKeep(6).build();
        ...
        this.toolCallLimitHook = ToolCallLimitHook.builder().runLimit(25).build();
        ...
        this.contextEditingInterceptor = ContextEditingInterceptor.builder().trigger(10000)
                .clearAtLeast(6000).keep(4).excludeTools("write_todos").build();
    ...
    private static SubAgentSpec createCritiqueAgent() {
        return SubAgentSpec.builder().name("critique-agent").description(
                        "Used to critique the final report. Provide information about "
                                + "how you want the report to be critiqued.")
                .systemPrompt(Prompts.SUB_CRITIQUE_PROMPT).enableLoopingLog(true)
                .build();
    }
    ...
    private static SubAgentSpec createResearchAgent(List<ToolCallback> toolsFromMcp) {
        return SubAgentSpec.builder().name("research-agent").description(
                        "Used to research in-depth questions. Only give one topic at a time. "
                                + "Break down large topics into components and call multiple research agents "
                                + "in parallel for each sub-question.")
                .systemPrompt(Prompts.SUB_RESEARCH_PROMPT).tools(toolsFromMcp)
                .enableLoopingLog(true).build();
    }
    ...
    public ReactAgent getResearchAgent(List<ToolCallback> toolsFromMcp) {
        ...
        return ReactAgent.builder().name("DeepResearchAgent").model(chatModel)
                .tools(toolsFromMcp).systemPrompt(systemPrompt).enableLogging(true)
                // 配置所有拦截器：任务列表、文件系统、大结果驱逐、工具调用补丁、
                // 上下文编辑、工具重试，以及子代理拦截器
                .interceptors(todoListInterceptor, filesystemInterceptor,
                        largeResultEvictionInterceptor,
                        patchToolCallsInterceptor,
                        contextEditingInterceptor, toolRetryInterceptor,
    ...
    private Interceptor subAgentAsInterceptors(List<ToolCallback> toolsFromMcp) {
        ...
        SubAgentInterceptor.Builder subAgentBuilder = SubAgentInterceptor.builder()
                ...
                // 子代理默认使用的拦截器
                .defaultInterceptors(todoListInterceptor, filesystemInterceptor,
                        contextEditingInterceptor,
                        patchToolCallsInterceptor,
                        largeResultEvictionInterceptor)
                // 子代理默认使用的钩子
                .defaultHooks(humanInTheLoopHook, summarizationHook,
                        toolCallLimitHook, shellToolAgentHook)
                // 添加研究代理
